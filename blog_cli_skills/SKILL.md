...
如果系统找不到 `blog-cli`，告诉用户需要安装blog-cli：
...
3. 登录：
    - 先使用`blog-cli login-status`校验登录状态 未登录使用 `blog-cli login-browser`登录。
...
```bash
blog-cli post update <文章ID> --title "<标题>" --content-file <正文文件路径> --summary "<摘要>" --status 1 --tags "标签1,标签2"
```
